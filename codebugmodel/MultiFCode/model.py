...
class Attention_N_module(nn.Module):
    def __init__(self, data_type, ori_dim, map_dim, concat=False):
        ...
        self.attention_F = nn.ModuleList(
            [nn.Sequential(*copy.deepcopy(ast_cfg_pdg_text_F)) for _ in range(self.num_modules)])
        self.attention = nn.ModuleList(
            [nn.Sequential(*copy.deepcopy(ast_cfg_pdg_text_att)) for _ in range(self.num_modules)])
