...
def run_command_with_retries(command, max_retries=10):
    ...
    while attempt < max_retries:
        ...
        if result.returncode == 0:
            ...
            print('-' * 10)
            print(f"The command-{command}-execution failed, with a return code of {result.returncode}, and the attempt count is now {attempt + 1}")
            print('-' * 10)
            attempt += 1
            time.sleep(5)
    print('-' * 10)
...
def readJSONDataAndGeneratePDG(config: DictConfig, datasetName, export_format):
    ...
    for splitName in split_name:
        temp_function_dir_path, output_dir_path, output_dot_max_path, output_pickle_path = generate_dir(config, splitName,
                                                                                              datasetName,
                                                                                              'all',
                                                                                              export_format)
