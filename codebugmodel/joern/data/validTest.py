...
def try_some_property(file_path):
    ...
    try:
        if pyg_data.num_nodes != len(pyg_data.x):
            with my_lock:
                global count
                count += 1
            raise Exception('num_nodes != len(x)')
        if pyg_data.num_edges is None or pyg_data.num_nodes is None:
            raise Exception('num_edges or num_nodes is None')
